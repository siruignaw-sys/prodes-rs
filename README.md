...
- **Shell electrostatics geometry** (`calculations/shell.rs`): the geometric core of `prodes`'s "Shell" electrostatics feature family. For each of `n_points` directions sampled evenly around the structure's centroid (a sunflower-sphere distribution), a plane is built just beyond the surface in that direction; every charged atom is projected onto it, and its contribution to the potential there is computed through a two-dielectric-media path (protein interior, dielectric 4; solvent, dielectric 80), split at the atom's exit point through the surface, found via a spatial-hash grid walk. Per-direction potentials are aggregated into a `ShellFeatures` struct: max, min, and full summary statistics (mean/median/trimean/std) across all directions, plus separate counts and statistics for the positive- and negative-potential subsets. Each geometric primitive (`find_plane`, `project_point`, `move_point`, `maximal_distance`, `required_distance`, `find_exit`) is unit-tested against hand-computed axis-aligned cases, and `map_ep_to_plane`'s output is checked against a value independently derived from the underlying two-media Coulomb formula, the same way the flat surface-electrostatics feature above was validated. **Not yet wired into the CLI**: `calculate_shell_features` isn't called from `main.rs`, so no run currently produces Shell-feature output columns, and unlike SASA and the flat electrostatics feature, this one hasn't yet been cross-checked against a live run of the real Python `prodes` on an actual structure. The tests so far confirm the geometry and physics are internally consistent, not that they numerically match the reference tool.

Not yet implemented:

- Wiring `calculate_shell_features` into `main.rs`'s per-pH CLI output (the underlying computation exists and is tested; it just isn't part of the `prodes-rs` binary's output yet).
...
The protein's chain A is used by default (falling back to the first chain present if none is named "A"). The isoelectric point is pH-independent and repeated on every row for convenience; the surface electrostatics feature is recomputed per pH value. Shell electrostatics (`calculations/shell.rs`) are computed and tested but not yet part of this CLI output; see "Status" above.
...
└── calculations/
    ...
    └── shell.rs                     Shell electrostatics: plane construction, atom projection,
                                      two-media potential, per-direction summary statistics
                                      (implemented and tested; not yet wired into the CLI)
```
